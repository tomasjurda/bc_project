...
class Level:
    def __init__(self, tmx_file: str, quests):
        ...
        with open(join("data", "npcs.json"), "r", encoding="utf-8") as f:
            ...
            for inter in self.interact_sprites:
                if self.player.rect.colliderect(inter.rect):
                    if inter.type == "door" or inter.type == "invisible_door":
                        ...
                        return {"target": location, "spawn": spawn, "options": options}
                    else:
                        print(inter.type)
                        return None
    ...
    def spawn_entities(self, options):
        for spawn, type in self.npc_spawn_positions:
            if type == "smart_hostile":
                if options == "TREE":
                    HostileNPC(
                        ...
                        pygame.image.load(
                            join("graphics", "models", "Player_BLUE.png")
                        ).convert_alpha(),
                        self.collision_sprites,
                        self.player,
                        brain_type="TREE",
            ...
            elif type in self.npc_database:
                npc_data = self.npc_database[type]
                NonHostileNPC(
                    (spawn[0], spawn[1]),
                    (self.all_sprites, self.enemy_sprites),
                    pygame.image.load(
    ...
    def update(self, dt):
        ...
        if self.dialog_ui.active:
            ...
            # Kontrola provokace z UI
            # if self.dialog_ui.provoked_trigger:
            #    self.provoke_npc(self.dialog_ui.current_npc)
        self.combat_handler.check_hits(self.player, self.enemy_sprites)
