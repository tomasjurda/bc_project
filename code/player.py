...
class Player(I_Entity):
    def __init__(self, pos, groups ,sprite_sheet):
        ...
        self.states = {
            "IDLE"          : { "state" : Player_Idle()             ,   "stamina_cost"  : 0.0   ,   "animation" :   { 'down' : self.load_frames(0, 6 , False), 'right' :  self.load_frames(1, 6, False), 'left' : self.load_frames(1, 6, True),'up' : self.load_frames(2, 6, False)}},
            "RUN"           : { "state" : Player_Run()              ,   "stamina_cost"  : 0.0   ,   "animation" :   { 'down' : self.load_frames(3, 6 , False), 'right' :  self.load_frames(4, 6, False), 'left' : self.load_frames(4, 6, True),'up' : self.load_frames(5, 6, False)}},
            "DODGE"         : { "state" : Dodge(1,4)                ,   "stamina_cost"  : 3.0   ,   "animation" :   { 'down' : self.load_frames(3, 6 , False), 'right' :  self.load_frames(4, 6, False), 'left' : self.load_frames(4, 6, True),'up' : self.load_frames(5, 6, False)}},
            "LIGHT_ATTACK"  : { "state" : Light_Attack(2,3)         ,  "stamina_cost"   : 3.0   ,   "animation" :   { 'down' : self.load_frames(6, 5 , False), 'right' :  self.load_frames(7, 5, False), 'left' : self.load_frames(7, 5, True),'up' : self.load_frames(8, 5, False)}},
